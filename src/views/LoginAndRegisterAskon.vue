    ...
    <div>
      <div>
        <div :class="action == 'login' ? 'direita' : 'esquerda'">
          <div
            :class="action == 'login' ? 'img-direita' : 'img-esquerda'"
          ></div>
        </div>
      </div>
      <div v-if="action == 'login'" class="login" key="login">
        <div class="container-login">
          ...
          />
          <p class="sub-texto" @click.prevent="action = 'recuperar'">
            esqueci minha senha
          </p>
          <button class="btn-login" @click="SubmitLogin">Entrar</button>
          <p class="sub-texto">Novo no site?</p>
          <button class="btn-login" @click.prevent="ChangeAction('registrar')">
            Registrar
          </button>
        </div>
      </div>
      ...
      <div class="recuperar" v-if="action == 'recuperar'" key="recuperar">
        <div class="box-recuperar">
          ...
          <div class="box-texto-recuperar">
            <input type="text" class="input-login" placeholder="Email" />
            <button class="btn-login" @click.prevent="ChangeAction('codigo')">
              enviar codigo
            </button>
          </div>
          <div class="box-texto-recuperar" v-if="action == 'codigo'">
            <input
              ...
              pattern="[0-9]+"
            />
            <button class="btn-login" @click.prevent="ChangeAction('login')">
              Entrar
            </button>
          </div>
        </div>
